...
export interface AttributeOptions<M extends Model = Model> {
  ...
  /**
   * A SQL expression that defines this column as a generated (computed) column.
   * Must be a {@link Literal} created via `sql.literal(...)`.
   *
   * Generated columns are read-only and their values are automatically computed by the database.
   *
   * @example
   * ```ts
   * {
   ...
   * ```
   */
  generatedAs?: Literal | undefined;

  /**
   * The storage mode for a generated column.
   * - `'STORED'`: The value is computed on write and stored on disk.
   * - `'VIRTUAL'`: The value is computed on read and not stored on disk.
   *
   * Defaults to `'STORED'` if {@link generatedAs} is set and this is not specified.
   * Not all dialects support `'VIRTUAL'` generated columns.
   */
  generatedColumn?: 'STORED' | 'VIRTUAL' | undefined;
