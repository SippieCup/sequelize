  ...
  it('produces a query to describe a table', () => {
    expectsql(() => queryGenerator.describeTableQuery('myTable'), {
      ...
      postgres: `SELECT
        ...
        (SELECT array_agg(e.enumlabel) FROM pg_catalog.pg_type t JOIN pg_catalog.pg_enum e ON t.oid=e.enumtypid JOIN pg_catalog.pg_namespace n ON n.oid=t.typnamespace WHERE t.typname=c.udt_name AND n.nspname=c.udt_schema) AS "special",
